...
TEXT TO m.lsql NOSHOW ADDITIVE TEXTMERGE PRETEXT 7
	SELECT TR_HEADER.ID,
			...
			(
				SELECT SUM(JUMLAH) FROM tr_bayar
				WHERE tr_bayar.tr_id = tr_header.tr_id AND tr_bayar.tr_number = tr_header.tr_number AND tr_bayar.tipe_bayar = 1 AND tr_bayar.status = 3
			) AS CASH,			
			(
				SELECT SUM(JUMLAH) FROM tr_bayar
				WHERE tr_bayar.tr_id = tr_header.tr_id AND tr_bayar.tr_number = tr_header.tr_number AND tr_bayar.tipe_bayar = 2 AND tr_bayar.status = 3
			) AS TRANSFER
	FROM TR_HEADER
...
ELSE
	IF USED('temp_header')
		...
		IF RECCOUNT() > 0	
			...
			_title                  = IIF(temp_header.tr_id = 1 , 'Beli/Buy',IIF(temp_header.tr_id = 2, 'Jual/Sale', '')) 
				...
				IF USED('temp_detail')
					...
					IF RECCOUNT() > 0
						...
						IF RECCOUNT() > 4
							...
							SCAN 
								NREK = NREK + 1
								REPLACE ID WITH NREK
								subtot = subtot + (price*qty)
							ENDSCAN  
							SELECT * FROM temp_detail WHERE id BETWEEN 1 .AND. 5 INTO CURSOR temp_detail1 READWRITE ORDER BY valas_id,price ASC 
							SELECT * FROM temp_detail WHERE id BETWEEN 6 .AND. 10 INTO CURSOR temp_detail2 READWRITE ORDER BY valas_id,price ASC
							SELECT * FROM temp_detail WHERE id BETWEEN 11 .AND. 15 INTO CURSOR temp_detail3 READWRITE ORDER BY valas_id,price ASC 
							FOR I = 1 TO 3
								ni = ALLTRIM(STR(i))
